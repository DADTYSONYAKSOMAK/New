...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@P_I_V_1"))
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@VR_11_V"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await sython.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : الرسايل المحفوظه :مطور السورس @P_I_V_1"
    )
